     ...
     "duties": [
       {
         ...
         "code": "713",                  // flight no | OFF RQOFF VAC RDO RESV S1 S2 SBY SBY-A SBY-B L/O TRAIN JCRM/C(ground)
...
(function (root) {
  ...
  function isSim(code) { return /^SIM/i.test(String(code || '').trim()); }
  ...
  function categoryOf(duty) {
    ...
    if (raw === 'OAL') return 'train';        // positioning on another carrier (crew rows carry role DH)
    if (/^JCRM/.test(raw) || /\/C$/.test(raw)) return 'ground'; // ground class (CRM etc.)
    if (isSim(raw)) return 'sim';
  ...
  function roleSuffix(role) {
    ...
    if (r === 'DH') return ' (DH)';
    if (r === 'TFO') return ' (TFO)';
    if (r === 'EX' || r === 'EXTRA' || r === 'F2EX') return ' (EX)';
    return '';
  }
  ...
  function build(input) {
    ...
    // crewSet (double-crew F1/F2) auto-EX: group consecutive flight legs of one trip
    // (layovers act as connectors; any other duty breaks the trip). Within a crewSet run,
    // F2 operates only the LAST (inbound) leg; F1 operates only the FIRST (outbound) leg.
    // Non-operating legs get role 'EX' so they render "(EX)" and are excluded from logging.
    (function applyCrewSet() {
      var i = 0;
      while (i < norm.length) {
        if (norm[i].cat !== 'flight' || !norm[i].raw.crewSet) { i++; continue; }
        var cs = String(norm[i].raw.crewSet).toUpperCase();
        var legs = [], j = i;
        while (j < norm.length) {
          var c = norm[j];
          if (c.cat === 'flight') {
            if (!c.raw.crewSet || String(c.raw.crewSet).toUpperCase() !== cs) break;
            legs.push(c); j++;
          } else if (c.cat === 'layover') { j++; }
          else break;
        }
        if (legs.length >= 2) {
          if (cs === 'F2') { for (var k = 0; k < legs.length - 1; k++) legs[k].raw.role = 'EX'; }
          else if (cs === 'F1') { for (var k2 = 1; k2 < legs.length; k2++) legs[k2].raw.role = 'EX'; }
        }
        i = (j > i) ? j : i + 1;
      }
    })();

    var SELF_EMP = 'T230604';
    var KRNAME = {
      GMP: '김포', CJU: '제주', CJJ: '청주', YNJ: '연길', ICN: '인천', NRT: '도쿄·나리타',
      KIX: '오사카·간사이', FUK: '후쿠오카', HKG: '홍콩', BKK: '방콕', KWJ: '광주', WUH: '우한',
      CXR: '나트랑', KLO: '칼리보', PUS: '부산', TAE: '대구', RSU: '여수', KPO: '포항', HND: '도쿄·하네다'
    };
    function aptLabel(c) { c = String(c || '').toUpperCase(); return KRNAME[c] ? (c + '(' + KRNAME[c] + ')') : c; }
    function clockStr(s) { s = String(s == null ? '' : s); return /^\d{4}$/.test(s) ? (s.slice(0, 2) + ':' + s.slice(2)) : s; }
    function crewLine(s) {
      var t = String(s || '').trim().split(/\s+/);
      if (/^T\d+$/.test(t[0])) {
        var emp = t[0], name = t[1], rest = t.slice(2).join(' ');
        return '• ' + name + ' — ' + rest + ' [' + emp + ']' + (emp === SELF_EMP ? '  ★본인' : '');
      }
      var nm = t[0], rs = t.slice(1).join(' ');
      return '• ' + nm + (rs ? ' — ' + rs : '');
    }
    function roleLabel(r) {
      r = String(r || '').toUpperCase();
      if (r === 'EX' || r === 'EXTRA' || r === 'F2EX') return 'EX (2편조·비조종 탑승)';
      if (r === 'DH') return 'DH (deadhead)';
      if (r === 'TFO') return 'TFO';
      return r;
    }
    function descOf(d, extra) {
      ...
      if (tag) L.push(tag);
      var body = [];
      if (d.from && d.to) body.push('구간: ' + aptLabel(d.from) + ' → ' + aptLabel(d.to));
      if (d.std != null || d.sta != null) body.push('STD ' + clockStr(d.std) + ' / STA ' + clockStr(d.sta) + ' (KST' + (d.bnote ? ' · (B)블록타임 우선 적용' : '') + ')');
      if (d.reg) body.push('기재(A/C): ' + d.reg);
      if (d.role) body.push('본인 임무: ' + roleLabel(d.role));
      if (body.length) L.push(body.join('\n'));
      if (Array.isArray(d.crew) && d.crew.length) L.push('[ 편조 / Crew ]\n' + d.crew.map(crewLine).join('\n'));
    ...
    norm.forEach(function (n) {
      ...
      if (cat === 'off' || cat === 'off-resv') {
        ...
        ev = {
          cat: 'train', allDay: false,
          summary: String(d.code || 'TRAIN').toUpperCase() + ' ' + routeStr(d) + roleSuffix(d.role || 'DH'),
          colorId: COLOR.train,
        ...
        var simStart = (n.std != null ? n.std : (n.su != null ? n.su : 0));
        var simEnd = (n.sta != null ? n.sta : simStart);
        var e4s = endParts(d.date, simStart, simEnd);
        ev = {
          cat: 'sim', allDay: false,
          summary: String(d.code).toUpperCase() + (d.rem ? ' (' + d.rem + ')' : ''),
          colorId: COLOR.eval, startDateTime: dt(d.date, simStart), endDateTime: dt(e4s.date, e4s.min),
          description: descOf(d, '')
        };
      } else if (cat === 'ground') {
        var gs = (n.su != null ? n.su : n.std) || 0, ge = (n.sta != null ? n.sta : gs);
        var eg = endParts(d.date, gs, ge);
        ev = {
          cat: 'train', allDay: false, summary: String(d.code).toUpperCase(),
          colorId: COLOR.train, startDateTime: dt(d.date, gs), endDateTime: dt(eg.date, ge===gs?eg.min:eg.min),
          description: descOf(d, '')
        };
      } else if (cat === 'standby') {
    ...
    var showCats = { flight: 1, train: 1, sim: 1, eval: 1 };
    var firstByDate = {};
  ...
  function diff(existing, events) {
    ...
    existing.forEach(function (e) {
      ...
      if (u) (byUid[u] = byUid[u] || []).push(e);   // keep every copy: a UID can repeat if an earlier sync was interrupted
      var k = remoteStartKey(e); (byStart[k] = byStart[k] || []).push(e);
    });
    var toInsert = [], toPatch = [], toDelete = [], seen = {};
    function delRec(e) {
      return { id: e.id, summary: e.summary, start: (e.start && (e.start.dateTime || e.start.date)) || '' };
    }
    events.forEach(function (ev) {
      seen[ev.uid] = 1;
      var list = byUid[ev.uid] || [];
      var found = list[0];
      // Same UID appearing more than once means a previous Apply inserted twice (retry after a
      // partial/rate-limited run). Keep the first copy and delete the rest, otherwise they are
      // unreachable forever: the delete pass below skips any UID present in `seen`.
      for (var j = 1; j < list.length; j++) {
        if (!used[list[j].id]) { used[list[j].id] = 1; toDelete.push(delRec(list[j])); }
      }
    ...
    existing.forEach(function (e) {
      var u = uidFromDescription(e.description);
      if (u && !seen[u] && !used[e.id]) toDelete.push(delRec(e));
    });
