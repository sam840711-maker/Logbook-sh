...
(function (root) {
  ...
  function categoryOf(duty) {
    ...
    if (raw === 'RESV') {
      var sameRoute = duty.from && duty.to && String(duty.from).toUpperCase() === String(duty.to).toUpperCase();
      return sameRoute ? 'off-resv' : 'resv';   // GMP-GMP = all-day reserve
    }
  ...
  function build(input) {
    ...
    var norm = duties.map(function (d, i) {
      ...
      return {
        ...
        anchor: (su != null ? su : (std != null ? std : (sta != null ? sta : 0)))
      };
    }).sort(function (a, b) {
    ...
    Object.keys(firstByDate).forEach(function (d0) {
      ...
      showEvents.push({
        ...
        description: '', _date: d0, _anchor: s, _code: 'SU'
      });
    });
  ...
  // Minimal ICS (v0.6b). Times are floating local with TZID=Asia/Seoul.
  function toIcs(res) {
    function esc(s) { return String(s || '').replace(/\\/g, '\\\\').replace(/;/g, '\\;').replace(/,/g, '\\,').replace(/\n/g, '\\n'); }
    function stampLocal(s) { return s.replace(/[-:]/g, ''); } // 'YYYY-MM-DDTHH:MM:00' -> 'YYYYMMDDTHHMM00'
    function stampDate(s) { return s.replace(/-/g, ''); }
    var L = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//SH Pilot Logbook//roster ' + ENGINE_VER + '//EN', 'CALSCALE:GREGORIAN'];
    (res.events || []).forEach(function (e) {
      L.push('BEGIN:VEVENT');
      L.push('UID:' + e.uid);
      if (e.allDay) {
        L.push('DTSTART;VALUE=DATE:' + stampDate(e.startDate));
        L.push('DTEND;VALUE=DATE:' + stampDate(e.endDate));
      } else {
        L.push('DTSTART;TZID=Asia/Seoul:' + stampLocal(e.startDateTime));
        L.push('DTEND;TZID=Asia/Seoul:' + stampLocal(e.endDateTime));
      }
