...
(function (root) {
  ...
  // PDF engine build. Bump ONLY when this file (logbook-pdf.js) changes.
  // Shown in the PDF header as "engine vX" so a stale/not-uploaded engine is immediately visible
  // even if the app/cache version (version.js) was bumped.
  var ENGINE_VER = 'v0.6-spread';
  ...
  function easaCols() {
    return [
      ...
      { lab: 'REMARKS', w: 0, a: 'l', wrap: true, get: function (f) { return remark(f); } }
    ];
  }
  function faaCols() {
    return [
  ...
  function fstdCols() {
    // DATE intentionally omitted — the ledger's first DATE column already carries the session date.
    return [
      { grp: 'FSTD SESSION', lab: 'TYPE', w: 14, a: 'l', fstd: true, get: function () { return ''; }, simGet: function (s) { return s.type || ''; } },
      { grp: 'FSTD SESSION', lab: 'TOTAL', w: 12, a: 'r', fstd: true, get: function () { return ''; }, simGet: function (s) { return fmtHMz(s.total); }, simSum: function (s) { return s.total || 0; } }
    ];
  }
  ...
  function jcabCols() {
    ...
    return [
      ...
      { lab: 'REMARKS', w: 0, a: 'l', wrap: true, get: function (f) { return remark(f); }, simGet: function (s) { return simRemark(s); } }
    ]);
  }
  function gcaaCols() {
    ...
    return [
      { lab: 'DATE', w: 16, a: 'l', get: function (f) { return ddmmyy(f.date); }, simGet: function (s) { return ddmmyy(s.date); } },
      { lab: 'FLIGHT', w: 12, a: 'l', get: function (f) { return f.fno || ''; } },
      { grp: 'DEPARTURE', lab: 'PLACE', w: 12, a: 'l', get: function (f) { return f.dep || ''; } },
      { grp: 'DEPARTURE', lab: 'TIME', w: 10, a: 'c', get: function (f) { return f.off != null ? clk(f.off) : ''; } },
      { grp: 'ARRIVAL', lab: 'PLACE', w: 12, a: 'l', get: function (f) { return f.arr || ''; } },
      { grp: 'ARRIVAL', lab: 'TIME', w: 10, a: 'c', get: function (f) { return f.on != null ? clk(f.on) : ''; } },
      ...
      { grp: 'LANDINGS', lab: 'NGT', w: 8, a: 'r', sum: function (f) { return f.nLdg || 0; }, isInt: true, get: function (f) { return f.nLdg || ''; } },
      { grp: 'OPERATIONAL TIME', lab: 'NIGHT', w: 11, a: 'r', sum: function (f) { return f.night || 0; }, get: function (f) { return fmtHM(f.night); } },
      { grp: 'OPERATIONAL TIME', lab: 'ACTUAL INST', w: 12, a: 'r', sum: function (f) { return f.actual || 0; }, get: function (f) { return fmtHM(f.actual); } },
      { grp: 'OPERATIONAL TIME', lab: 'X-CTRY', w: 12, a: 'r', sum: function (f) { return f.xc ? (f.total || 0) : 0; }, get: function (f) { return f.xc ? fmtHM(f.total) : ''; } },
  ...
  function build(jsPDF, opts) {
    opts = opts || {};
    // Header tag shows the ENGINE's own version (not the injected app version), so a stale engine is visible.
    var BUILD = 'engine ' + ENGINE_VER;
    ...
    // Font-aware width measuring/clipping (mirrors txtKR's font path so measured width == drawn width).
    function measW(s) {
      if (hasKR(s) && KRFONT) { var pf = doc.getFont(); doc.setFont(KRFONT, 'normal'); var w = doc.getTextWidth(s); doc.setFont(pf.fontName || 'helvetica', pf.fontStyle || 'normal'); return w; }
      return doc.getTextWidth(s);
    }
    function dispStr(txt) { txt = String(txt); return (hasKR(txt) && !KRFONT) ? kr2latin(txt) : txt; }
    function fitOne(txt, maxw) {
      var ds = dispStr(txt);
      if (measW(ds) <= maxw) return ds;
      var ell = '\u2026';
      while (ds.length > 1 && measW(ds + ell) > maxw) ds = ds.slice(0, -1);
      return ds + ell;
    }
    function wrapKR(txt, maxw, maxLines) {
      var ds = dispStr(txt);
      if (measW(ds) <= maxw) return [ds];
      var lines = [], rest = ds, ell = '\u2026';
      for (var ln = 0; ln < maxLines; ln++) {
        if (measW(rest) <= maxw) { lines.push(rest); rest = ''; break; }
        if (ln === maxLines - 1) { var s = rest; while (s.length > 1 && measW(s + ell) > maxw) s = s.slice(0, -1); lines.push(s + ell); rest = ''; break; }
        var lo = 1, hi = rest.length, fit = 1;
        while (lo <= hi) { var mid = (lo + hi) >> 1; if (measW(rest.slice(0, mid)) <= maxw) { fit = mid; lo = mid + 1; } else hi = mid - 1; }
        var bp = fit;
        for (var k = fit; k > Math.max(1, fit - 8); k--) { var ch = rest[k - 1]; if (ch === ' ' || ch === '/' || ch === '-') { bp = k; break; } }
        lines.push(rest.slice(0, bp).replace(/\s+$/, ''));
    ...
    function header() {
      ...
      doc.text('Generated ' + new Date().toISOString().slice(0, 10) + '  ·  ' + BUILD + '   Page ' + pageNo + ' / ' + (totalPages + 1), PW - M, 10, { align: 'right' });
      doc.setFontSize(7);
    ...
    function cell(x, y, w, h, txt, align, wrap) {
      doc.rect(x, y, w, h);
      if (txt === '' || txt == null) return;
      txt = String(txt);
      var pad = 1.0, maxw = w - pad * 2; if (maxw < 2) maxw = w;
      var tx = align === 'r' ? x + w - pad : align === 'c' ? x + w / 2 : x + pad;
      var opt = { align: align === 'r' ? 'right' : align === 'c' ? 'center' : 'left' };
      if (wrap) {
        var lines = wrapKR(txt, maxw, 2), lineH = 2.3;
        var first = y + h / 2 - (lines.length - 1) * lineH / 2 + 1;
        for (var i = 0; i < lines.length; i++) txtKR(lines[i], tx, first + i * lineH, opt);
      } else {
        var ds = dispStr(txt), wd = measW(ds);
        if (wd > maxw && wd > 0) {
          var size = 6 * maxw / wd;
          if (size < 4.0) { doc.setFontSize(4.0); txtKR(fitOne(txt, maxw), tx, y + h / 2 + 1, opt); doc.setFontSize(6); }
          else { doc.setFontSize(size); txtKR(ds, tx, y + h / 2 + 1, opt); doc.setFontSize(6); }
        } else {
          txtKR(ds, tx, y + h / 2 + 1, opt);
        }
      }
    }
    function dataRow(entry, y) {
      ...
      cols.forEach(function (c, idx) {
        ...
        cell(x, y, c.w, rowH, val, c.a, c.wrap);
    ...
    /* ---- 2-page L/R spread (EASA / GCAA): cols 1-8 on the left page, 9-12 on the right,
       row-aligned (fixed row height) so the two pages read as a facing spread. ---- */
    function splitLR(ca) {
      var li = -1; for (var i = 0; i < ca.length; i++) { if (ca[i].grp === 'LANDINGS') li = i; }
      if (li < 0) return { left: ca.slice(), right: [] };
      return { left: ca.slice(0, li + 1), right: ca.slice(li + 1) };
    }
    function distribute(ca) {
      var fx = 0, fl = null; ca.forEach(function (c) { if (c.w) fx += c.w; else fl = c; });
      if (fl) { var rmin = 26; if (fx + rmin > usable) { var sc = (usable - rmin) / fx; ca.forEach(function (c) { if (c.w) c.w *= sc; }); fx = usable - rmin; } fl.w = Math.max(rmin, usable - fx); }
      else { var t = 0; ca.forEach(function (c) { t += c.w; }); if (t > 0) { var s = usable / t; ca.forEach(function (c) { c.w *= s; }); } }
    }
    function drawHeaderRowC(ca, hh) {
      var yy = top, x = x0; doc.setFontSize(6); doc.setFont('helvetica', 'bold'); doc.setLineWidth(0.2);
      var i = 0;
      while (i < ca.length) {
        var c = ca[i];
        if (c.grp) {
          var ws = 0, j = i; while (j < ca.length && ca[j].grp === c.grp) { ws += ca[j].w; j++; }
          doc.rect(x, yy, ws, hh / 2); hdrCell(c.grp, x + ws / 2, yy, hh / 2, ws);
          var xx = x; for (var k = i; k < j; k++) { doc.rect(xx, yy + hh / 2, ca[k].w, hh / 2); hdrCell(ca[k].lab, xx + ca[k].w / 2, yy + hh / 2, hh / 2, ca[k].w); xx += ca[k].w; }
          x += ws; i = j;
        } else { doc.rect(x, yy, c.w, hh); hdrCell(c.lab, x + c.w / 2, yy, hh, c.w); x += c.w; i++; }
      }
    }
    function dataRowC(entry, yy, ca, run2, pv2) {
      var x = x0; doc.setFont('helvetica', 'normal'); doc.setFontSize(6);
      var isSim = entry.k === 's', o = entry.d;
      ca.forEach(function (c, idx) {
        var val = isSim ? (c.simGet ? c.simGet(o) : '') : c.get(o);
        cell(x, yy, c.w, rowH, val, c.a, c.wrap);
        var fn = isSim ? c.simSum : c.sum; if (fn) { var v = fn(o) || 0; run2[idx] = (run2[idx] || 0) + v; pv2[idx] = (pv2[idx] || 0) + v; }
        x += c.w;
      });
    }
    function emptyRowC(yy, ca) { var x = x0; ca.forEach(function (c) { doc.rect(x, yy, c.w, rowH); x += c.w; }); }
    function sumRowC(yy, label, vals, ca) {
      var x = x0; doc.setFont('helvetica', 'bold'); doc.setFontSize(6);
      var firstSum = -1; for (var i = 0; i < ca.length; i++) { if (summed(ca[i])) { firstSum = i; break; } }
      if (firstSum < 0) firstSum = ca.length;
      if (firstSum > 0) { var lw = 0; for (var k = 0; k < firstSum; k++) lw += ca[k].w; doc.rect(x, yy, lw, rowH); doc.text(label, x + 1, yy + rowH / 2 + 1); x += lw; }
      for (var c = firstSum; c < ca.length; c++) { var col = ca[c]; var txt = ''; if (summed(col)) txt = col.isInt ? String(vals[c] || 0) : fmtHMz(vals[c]); cell(x, yy, col.w, rowH, txt, summed(col) ? 'r' : col.a); x += col.w; }
    }
    function halfHeader(ca, hh, label, withSig) {
      doc.setDrawColor(40); doc.setTextColor(20);
      doc.setFont('helvetica', 'bold'); doc.setFontSize(11); doc.text(TITLES[format] || TITLES.EASA, x0, 10);
      doc.setFont('helvetica', 'normal'); doc.setFontSize(8);
      var who = (profile.name ? 'Name: ' + profile.name : '') + (profile.lic ? '   Licence: ' + profile.lic : ''); if (who) doc.text(who, x0, 15);
      doc.setFontSize(7); doc.text('All times UTC · HH:MM   ·   ' + BUILD, PW - M, 10, { align: 'right' });
      doc.text('Page ' + label, PW - M, 15, { align: 'right' });
      drawHeaderRowC(ca, hh);
      doc.setFontSize(7); doc.setTextColor(60); doc.text('Page ' + label, x0, PH - 4); doc.setTextColor(20);
      if (withSig) {
        var fy = PH - 4; doc.setDrawColor(120); doc.setTextColor(90); doc.setFont('helvetica', 'normal'); doc.setFontSize(7);
        var lx = PW - M - 64; doc.text('Signature:', lx, fy);
        var sx = lx + 17, sw = 44, sh = 11, sy = fy - sh + 2.5;
        if (profile.sig && /^data:image\/png/i.test(profile.sig)) { try { doc.addImage(profile.sig, 'PNG', sx, sy, sw, sh); } catch (e) { doc.setDrawColor(150); doc.line(sx, fy, sx + sw, fy); } }
        else { doc.setDrawColor(150); doc.line(sx, fy, sx + sw, fy); }
        doc.setTextColor(20); doc.setDrawColor(40);
      }
    }
    function renderSpread() {
      var LR = splitLR(cols); distribute(LR.left); distribute(LR.right);
      var hh = 11, bres = 22;
      var rpp = Math.floor((PH - top - hh - bres - M) / rowH); if (rpp < 1) rpp = 1;
      var lrun = {}, rrun = {};
      LR.left.forEach(function (c, i) { if (summed(c)) lrun[i] = 0; });
      LR.right.forEach(function (c, i) { if (summed(c)) rrun[i] = 0; });
      var pages = Math.max(1, Math.ceil(entries.length / rpp));
      function drawHalf(ca, run2, slice, label, withSig) {
        pageNo++; halfHeader(ca, hh, label, withSig);
        var pv = {}; ca.forEach(function (c, i) { if (summed(c)) pv[i] = 0; });
        var yy = top + hh;
        slice.forEach(function (e) { dataRowC(e, yy, ca, run2, pv); yy += rowH; });
        for (var k = slice.length; k < rpp; k++) { emptyRowC(yy, ca); yy += rowH; }
        sumRowC(yy, 'TOTAL THIS PAGE', pv, ca); yy += rowH;
        var prev = {}; ca.forEach(function (c, i) { if (summed(c)) prev[i] = (run2[i] || 0) - (pv[i] || 0); });
        sumRowC(yy, 'TOTAL PREVIOUS PAGES', prev, ca); yy += rowH;
        var cum = {}; ca.forEach(function (c, i) { if (summed(c)) cum[i] = run2[i]; });
        sumRowC(yy, 'TOTAL TIME', cum, ca); yy += rowH;
      }
      for (var p = 0; p < pages; p++) {
        var slice = entries.slice(p * rpp, (p + 1) * rpp);
        if (p > 0) doc.addPage();
        drawHalf(LR.left, lrun, slice, (p + 1) + '-L', false);
        doc.addPage();
        drawHalf(LR.right, rrun, slice, (p + 1) + '-R', true);
      }
    }

    if (format === 'EASA' || format === 'GCAA') {
      renderSpread();
    } else {
      header();
      y = top + hH; resetPage(); var onPage = 0;
      if (!entries.length) {
    ...
    doc.text('Generated ' + new Date().toISOString().slice(0, 10) + '  ·  ' + BUILD + '   Page ' + pageNo + ' / ' + pageNo, PW - M, 12, { align: 'right' });
    footer();
